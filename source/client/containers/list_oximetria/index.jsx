...
import Bread from "../../components/list_oximetria/bread";
import Title from "../../components/list_oximetria/title";
import Table from "../../components/list_oximetria/table";
import Spiner from "../../components/list_oximetria/spiner";
import Paginator from "../../components/list_oximetria/paginator";
import Modal from "../../components/list_oximetria/modalDelete";
...
/* Actions */
import * as listOximetriaActions from "../../actions/list_oximetria/index";

class ListOximetria extends Component {
  constructor(props) {
    super(props);
    this.state = {
      modal: false,
  ...
  async componentDidMount() {
    await this.props.actions.fetchOximetriaApi({
      pag: this.state.paginator
    });
    await this.props.actions.fetchOximetriaCountApi({});
  }

  async componentWillUnmount() {
    this.props.actions.resetListOximetria();
  }
  ...
  handleDelete = async event => {
    if (!this.props.loadingDelte) {
      ...
      /* await this.props.actions.deleteOximetriaApi(obj); */
    }
  };
...
const mapStateToProps = state => ({
  data: state.reducer.list_oximetria.data,
  loading: state.reducer.list_oximetria.loading,
  status: state.reducer.list_oximetria.status,
  dataDelete: state.reducer.list_oximetria.dataDelete,
  loadingDelete: state.reducer.list_oximetria.loadingDelete,
  statusDelete: state.reducer.list_oximetria.statusDelete,
  dataCount: state.reducer.list_oximetria.dataCount,
  loadingCount: state.reducer.list_oximetria.loadingCount,
  statusCount: state.reducer.list_oximetria.statusCount
});

const mapDispatchToProps = dispatch => ({
  actions: bindActionCreators(listOximetriaActions, dispatch)
});

ListOximetria.propTypes = {
